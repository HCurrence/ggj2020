...
public class WorkerList : MonoBehaviour {
    ...
    void Update() {
        var render = workerItems.Count != WorkManager.Inst.all_workers.Count;
        for (int i = 0; !render && i < workerItems.Count; i++) {
            render |= workerItems[i].Worker != WorkManager.Inst.all_workers[i];
        }
        if (render) {
            RenderWorkers();
        }
    ...
    public void RenderWorkers() {
        ...
        workerItems.Clear();
        for (var i = 0; i < WorkManager.Inst.all_workers.Count; i++) {
